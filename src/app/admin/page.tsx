                ...
                {devData && (<>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:16 }}>
                    {[
                      ...
                      { label:'Status', value: (securityData?.suspiciousIps?.length ?? 0) === 0 ? '✓ OK' : '⚠️ Pozor', color: (securityData?.suspiciousIps?.length ?? 0) === 0 ? '#4ade80' : '#f87171' },
                    ].map(s => (
                  ...
                  <div style={{ background:'rgba(0,0,0,0.4)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:16, overflow:'hidden' }}>
                    ...
                    <div style={{ maxHeight:300, overflowY:'auto' }}>
                      {(!securityData?.recentEvents?.length) ? (
                        <div style={{ padding:32, textAlign:'center', fontFamily:'Georgia,serif', fontSize:12, color:'rgba(245,238,242,0.2)' }}>✓ Žádné bezpečnostní události</div>
                  ...
                  <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:16, padding:'20px', marginBottom:16 }}>
                    ...
                    <button disabled={emailTestLoading} onClick={async () => {
                      ...
                      setEmailTestResult(d.ok ? '✓ Email odeslán!' : `✗ Chyba: ${d.error}`)
                    ...
                    {emailTestResult && (
                      <div style={{ marginTop:10, padding:'10px 14px', borderRadius:10, background: emailTestResult.startsWith('✓') ? 'rgba(74,222,128,0.1)' : 'rgba(248,113,113,0.1)', border:`1px solid ${emailTestResult.startsWith('✓')?'rgba(74,222,128,0.3)':'rgba(248,113,113,0.3)'}`, fontFamily:'Georgia,serif', fontSize:13, color: emailTestResult.startsWith('✓') ? '#4ade80' : '#f87171' }}>
                        {emailTestResult}
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:16, padding:'16px 20px' }}>
                    ...
                    {[
                      ...
                      ['Auto deploy', 'každý git push → Vercel build'],
                    ...
                    {[
                      { label:'Online teď', value: devData.online?.count ?? 0, color:'#4ade80', icon:'🟢' },
                      { label:'Push odběratelů', value: devData.push?.count ?? 0, color:'#FF6BA8', icon:'🔔' },
                      { label:'Recenzí čeká', value: Array.isArray(devData.reviews) ? devData.reviews.filter((r:any)=>r.status==='PENDING').length : 0, color:'#fbbf24', icon:'⭐' },
                      { label:'Aktivní výjezdů', value: Array.isArray(devData.checkins) ? devData.checkins.filter((c:any)=>!c.returnedAt).length : 0, color:'#f87171', icon:'🛡️' },
                    ].map(s => (
                      <div key={s.label} style={{ background:'rgba(255,255,255,0.04)', border:`1px solid ${s.color}25`, borderRadius:14, padding:'14px 12px', position:'relative', overflow:'hidden' }}>
                        <div style={{ position:'absolute', top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,transparent,${s.color}60,transparent)` }}/>
                        <div style={{ fontFamily:'Georgia,serif', fontSize:8, letterSpacing:2, color:'rgba(245,238,242,0.3)', textTransform:'uppercase', marginBottom:6 }}>{s.icon} {s.label}</div>
